    ...
    public class cSchoolsRepository : ISchoolsRepository, IDisposable
    {
            ...
            try
            {
                if (id != Guid.Empty)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(resourceParams.SearchQuery))
                    {
                        ...
                        queryableSchoolFacilities = queryableSchoolFacilities.Where(a => a.ValueApplied.ToString().ToUpper().Contains(searchQuery)
                    ...
                    var mappedResult = queryableSchoolFacilities.Select(x => new SchoolFacilitiesViewDto()
                    {
                        ...
                        ValueAupplied = x.ValueApplied,
                        CreatedBy = x.CreatedBy,
                        DateCreated = x.DateCreated,
                        //FacilitySettings
                        FacilitySettingPosition = x.FacilitySetting != null ? x.FacilitySetting.ItemNo : null,
        ...
        public async Task<SchoolsCreationDependecyDto> GetCreationDependencys()
        {
            ...
            try
            {
                ...
                var schoolCategorys = await _context.SchoolCategories.Select(x => new SchoolCategoryDto()
                {
                    Id = x.Id,
                    Code = x.Code,
                    Name = x.Name
